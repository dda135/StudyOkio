...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable {
   ...
   * {@code out}.
   * 将Buffer中offset开始的byteCount数据copy一份写入指定的OutputStream中
   */
  public Buffer copyTo(OutputStream out, long offset, long byteCount) throws IOException {
    ...
    // 从offset位置开始向OutputStream中写入数据
    for (; byteCount > 0; s = s.next) {
      //当初次进入循环的时候，因为一开始offset可能不是节点内部数据的整数量，所以起始位置要偏移
   ...
   * 从offset位置开始向指定的Buffer中写入byteCount的数据，这个对于指定的Buffer来说是添加操作
   * */
  public Buffer copyTo(Buffer out, long offset, long byteCount) {
    ...
    for (; byteCount > 0; s = s.next) {
      Segment copy = new Segment(s);//根据当前需要拷贝的数据节点拷贝一个节点
      copy.pos += offset;//初次进入offset要偏移起始位置，所以第一个拷贝的节点要是当偏移
      copy.limit = Math.min(copy.pos + (int) byteCount, copy.limit);//当前节点数据量
      if (out.head == null) {//如果Buffer中没有节点
        //当前节点为头结点
   ...
   *  将Buffer中byteCount字节数量的数据写入OutputStream中
   *  写入将会清空Buffer中的数据
   * */
  public Buffer writeTo(OutputStream out, long byteCount) throws IOException {
    ...
    while (byteCount > 0) {
      ...
      //注意这个的意义，特别是在最后的节点byteCount<8192的情况下，需要正确处理数据可用的起始位置
      s.pos += toCopy;
      //Buffer大小减少已经被读取的数据大小
      size -= toCopy;
      //剩余需要写入的字节数
      byteCount -= toCopy;

      if (s.pos == s.limit) {//回收当前被完全写入到OutputStream的节点
        Segment toRecycle = s;
  ...
  /**
   * 从InputStream中读取byteCount的数据放入Buffer中
   * @param in 需要被读取的流
   * @param byteCount 需要从流中读取的字节总数
   * @param forever true表示要完全读取in中的数据
     */
  ...
  @Override public String readString(long byteCount, Charset charset) throws EOFException {
    checkOffsetAndCount(size, 0, byteCount);//校验数据合理性
    ...
    Segment s = head;//定位到链表头
    if (s.pos + byteCount > s.limit) {//如果需要读取的字节量大于当前节点
      // If the string spans multiple segments, delegate to readBytes().
      // 这其实是说如果string包含了多个节点，那么通过readBytes进行代理操作才行
      return new String(readByteArray(byteCount), charset);
    }
    //否则直接通过当前节点构建一个String对象即可
    String result = new String(s.data, s.pos, (int) byteCount, charset);
    s.pos += byteCount;//标记当前可读取的起始位置增加
    size -= byteCount;//buffer中的总数据量减少

    if (s.pos == s.limit) {//如果当前节点被完全读取
      head = s.pop();//从链表中移除
      SegmentPool.recycle(s);//进入回收池中
    }
  ...
  @Override public byte[] readByteArray(long byteCount) throws EOFException {
    ...
    //int offset = 0;
    //while (offset < sink.length) {//遍历数组，实际上就是往里面填充数据
    //  int read = read(sink, offset, sink.length - offset);//再次委托read(byte[] sink,int start,int count)
    //  if (read == -1) throw new EOFException();//实际上当前并没有那么多字节量，抛出异常
    //  offset += read;//标记已经填充的数据
    //}
    readFully(result);
    return result;
  }
  ...
  @Override public int read(byte[] sink, int offset, int byteCount) {
    checkOffsetAndCount(sink.length, offset, byteCount);//校验数据合理性
    //定位到链表的头部
    Segment s = head;
    if (s == null) return -1;//buffer中没有数据，读取结束
    //当次可以读取的最大字节数，简单理解就是不能一次读取超过一个节点的数据
    ...
    if (s.pos == s.limit) {//如果当前节点完全读取
      head = s.pop();//从链表中移除
      SegmentPool.recycle(s);//进入回收池中
    }

    return toCopy;//当次读取的字节数量
  }
  ...
  @Override public void skip(long byteCount) throws EOFException {
    while (byteCount > 0) {//还有需要跳过的节点
      ...
      if (head.pos == head.limit) {//当前节点被完全跳过
        Segment toRecycle = head;
        head = toRecycle.pop();//从链表中移除
        SegmentPool.recycle(toRecycle);//进入回收池中
      }
    }
  }
  ...
  @Override public Buffer write(byte[] source, int offset, int byteCount) {
    if (source == null) throw new IllegalArgumentException("source == null");
    checkOffsetAndCount(source.length, offset, byteCount);//校验数据合理性

    int limit = offset + byteCount;//当前读取的结束位置
    while (offset < limit) {//判断写入操作是否结束
      //从当前buffer中获取一个可以写入的节点，可能是新建的，也可能是尾部有剩余空间的节点
      Segment tail = writableSegment(1);
      //当次可以写入的最大字节量
      int toCopy = Math.min(limit - offset, Segment.SIZE - tail.limit);
      //实际上就是将source中的数据拷贝一份到新建的节点中
      //此时将source中的数据写入到buffer当中
   ...
   * 这里要保证可以写入minimumCapacity的数据量
   * @param minimumCapacity 当前需要写入节点的字节数，此处是扩容，最多扩一个节点，所以不应该大于一个节点所存储的字节数
   */
  Segment writableSegment(int minimumCapacity) {
    ...
    if (head == null) {
      ...
      //尝试从循环池中获取，如果有则复用，否则新建
      //新建一个双向链表
      return head.next = head.prev = head;
    }
    //回到链表的尾部
    Segment tail = head.prev;
    //返回一个尾部节点保证可以写入minimumCapacity的数量，不然就扩容
    //当前尾部节点如果是新建节点，则没有内容，此时判断当前节点是否可以写入minimumCapacity数量的数据即可
    //否则直接扩容
    if (tail.limit + minimumCapacity > Segment.SIZE || !tail.owner) {
      //在原来链表的尾部和头部之间（双向链表）中插入一个节点
      tail = tail.push(SegmentPool.take()); // Append a new empty segment to fill up.
    }
    //总之最后保证可以写入minimumCapacity的数据量
    return tail;
  }
    ...
    while (byteCount > 0) {
      ...
      if (byteCount < (source.head.limit - source.head.pos)) {
        ...
        if (tail != null && tail.owner
          ...
          size += byteCount;
          return;
        } else {//当前节点无法写入byteCount字节数，需要扩容
      ...
      //将当前节点（segmentToMove）从链表中移除
      source.head = segmentToMove.pop();
      if (head == null) {//当前缓冲池没有数据
        //这里相当于把source中的某一个节点完全移动到buffer中
        //简单的实现了一个write操作
        head = segmentToMove;
        head.next = head.prev = head;
      } else {
        //否则要将source中的节点推到buffer的尾部
        Segment tail = head.prev;
