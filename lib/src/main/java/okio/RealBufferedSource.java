...
final class RealBufferedSource implements BufferedSource {
  //缓冲区，内部实际上就是一个双向链表，每一个节点代表一定量的数据
  //通过这种方式完成了流的链接，每个节点有两个标志，标识当前数据的有效读/写起点
  public final Buffer buffer = new Buffer();
  //Source本身是对InputStream的二次封装，内部的read本质上是对Buffer的操作
  ...
  /**
   * 尝试从Buffer中读取
   * @param sink 需要进行read操作的缓冲区
   * @param byteCount 需要读取的字节数
   * @return 当前一次读取的字节数
   * @throws IOException
     */
  @Override public long read(Buffer sink, long byteCount) throws IOException {
    ...
    if (buffer.size == 0) {//当前缓冲区为空
      //这段代码可以到Okio里面看，大概描述一下这个的作用
      //1.首先要进行时效性校验，Okio的流提供超时机制
      //2.扩容，保证buffer中的节点可以满足写入数据的操作，一次的read操作最多可读取8192也就是一个Segment的容量
      //3.对标志做处理，并且返回当前读取的字节数量
      //以上的节点值得是Buffer中sink的节点
      long read = source.read(buffer, Segment.SIZE);
      if (read == -1) return -1;//当前InputStream读取结束，没有字节数据
    }
    //当前实际读取的字节数
    long toRead = Math.min(byteCount, buffer.size);
    //从Buffer sink中将之前扩容的节点中的数据读取buffer中，完成这次读取操作
  ...
  @Override public String readString(long byteCount, Charset charset) throws IOException {
    ...
    //  if (source.read(buffer, Segment.SIZE) == -1) return false;
    //}
    //实际上就是判断当前缓冲区中数据量是否有byteCount字节量
    //如果没有则从Source封装的InputStream中一直读取，直到满足条件为止
    //如果InputStram读取结束也无法满足条件，抛出异常，此次读取结束
    require(byteCount);
    if (charset == null) throw new IllegalArgumentException("charset == null");
    //从buffer中读取byteCount数量的字节数
